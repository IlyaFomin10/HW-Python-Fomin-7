...
class Car:
    ...
    def go(self):
        return f'{self.name} is started and rides'
...
class TownCar(Car):
    ...
    def show_speed(self):
        if self.speed > 40:
            return f'Speed of {self.name} is {self.speed} and higher than ' \
                   f'allow for town car'
        else:
            return f'Speed of {self.name} is {self.speed} and normal for ' \
                   f'town car'
...
ford = SportCar(80, 'Black', 'Ford', False)
mazda = TownCar(90, 'White', 'Mazda', False)
yaz = WorkCar(50, 'Blue', 'YAZ', True)
kia = PoliceCar(100, 'Green',  'KIA', False)
print(f'{mazda.go()} - {mazda.show_speed()}')
print(kia.turn_left())
print(f'{yaz.name} is {yaz.color}')
